...
router.get('/fuel-prices', async (req, res) => {
  const forceRefresh = /^(1|true|yes)$/i.test(String(req.query.refresh || req.query.force || ''));
  let defaultData = buildDefaultPrices({ includeStale: false });

  if (forceRefresh || !defaultData) {
    try {
      await refreshDefaultFuelPrices();
      defaultData = buildDefaultPrices({ includeStale: false });
    } catch {
      defaultData = defaultData || buildDefaultPrices({ includeStale: true });
    }
  }
  ...
  const { prices, primarySourceKey, dataSources,
    priceDate, scrapedAt, cacheHit, ttlRemaining, isStale } = defaultData;
...
/* ==========================================================================
 * [ĐẦU MỐI API] GIÁ VÀNG
 * ========================================================================== */

/**
 * [GET] /api/gold-prices
 * Lấy giá vàng realtime từ API công khai, cache ngắn hạn ở backend.
 */
router.get('/gold-prices', async (req, res) => {
  const force = /^(1|true|yes)$/i.test(String(req.query.refresh || req.query.force || ''));
  const type = String(req.query.type || '').trim().toUpperCase();

  try {
    const result = await fetchGoldPrices({ force });
    const data = type ? result.data.filter((item) => item.code === type) : result.data;
    ...
    if (type && data.length === 0) {
      ...
      return res.status(404).json({
        ...
        status: 'not_found',
        message: {
          vi: `Không tìm thấy mã vàng "${type}".`,
          en: `Gold type "${type}" was not found.`,
        },
        availableTypes: result.data.map((item) => ({ code: item.code, name: item.name })),
      });
    }

    setCacheHeaders(res, Math.min(result.cacheTtlRemainingSeconds || 300, 300));
    return res.json({
      success: true,
      status: 'ok',
      disclaimer: DISCLAIMER,
      meta: {
        source: 'vang.today',
        sourceUrl: GOLD_API_URL,
        docsUrl: GOLD_API_DOCS_URL,
        fetchedAt: result.fetchedAt,
        updatedAt: result.updatedAt,
        priceDate: result.priceDate,
        priceTime: result.priceTime,
        cacheHit: result.cacheHit,
        cacheTtlRemainingSeconds: result.cacheTtlRemainingSeconds,
    ...
    return res.status(502).json({
      success: false,
      status: 'gold_source_error',
      message: {
        vi: 'Không thể lấy dữ liệu giá vàng realtime. Vui lòng thử lại sau.',
        en: 'Failed to fetch realtime gold prices. Please try again later.',
      },
      error: err.message,
    });
  }
...
router.get('/fuel-prices/:source', async (req, res) => {
  ...
  if (source === 'giaxanghomnay' && (forceRefresh || !data || stats.isStale)) {
    try {
      ...
      updateFuelPrices('giaxanghomnay', fresh);
      data = fresh;
      stats = getCacheStats('giaxanghomnay');
    } catch {
      // Nếu refresh lỗi, trả cache cũ bên dưới nếu còn.
    }
  }
...
router.get('/health', (req, res) => {
  ...
  Object.keys(SOURCES).forEach((src) => {
    ...
    if (!s.hit || s.isStale) allHealthy = false;
  });
