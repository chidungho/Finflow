...
async function run(baseUrl = process.env.API_BASE_URL || 'http://localhost:3000') {
  ...
  const nonStaleSources = perSource.filter((s) => !s.isStale).map((s) => s.source);
  const missing = nonStaleSources.filter((s) => !def.json.meta.dataSources.includes(s));
  assert.equal(missing.length, 0, `/api/fuel-prices missing non-stale sources: ${missing.join(', ')}`);
  ...
  run()
    .then((summary) => {
      ...
      summary.perSource.forEach((s) => {
        console.log(
          `  - ${s.source}: items=${s.items}, priceDate=${s.priceDate}, dateSource=${s.dateSource}, announcedAt=${s.announcedAt}, isStale=${s.isStale}`
        );
      });
      process.exit(0);
    })
