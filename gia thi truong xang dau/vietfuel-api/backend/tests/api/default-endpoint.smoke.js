...
async function run(baseUrl = process.env.API_BASE_URL || 'http://localhost:3000') {
  ...
  const gold = await fetchJson(`${baseUrl}/api/gold-prices`);
  assert.equal(gold.res.status, 200, '/api/gold-prices must return 200');
  assert.equal(gold.json.success, true, '/api/gold-prices success must be true');
  assert(Array.isArray(gold.json.data), '/api/gold-prices data must be an array');
  assert(gold.json.data.length > 0, '/api/gold-prices data must not be empty');
