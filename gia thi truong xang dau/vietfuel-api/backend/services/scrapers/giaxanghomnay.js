...
const { scrapeGiaxanghomnayViaHttp } = require('./giaxanghomnay-http');
...
async function scrapeGiaxanghomnay() {
  ...
  try {
    const result = await scrapeGiaxanghomnayViaHttp();
    logger.info(`[Scraper:GiaXangHomNay] HTTP thành công. ${result.prices.length} sản phẩm. priceDate=${result.priceDate} (${((Date.now() - start) / 1000).toFixed(2)}s)`);
    return result;
  } catch (httpErr) {
    logger.warn(`[Scraper:GiaXangHomNay] HTTP lỗi: ${httpErr.message}. Thử fallback Playwright...`);
  }
