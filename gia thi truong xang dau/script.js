/* ===================================================
   Vietnam Fuel API - inlined script
...
function getBackendApiBaseUrl() {
  const isBackendPage = window.location.protocol.startsWith('http');
...
function renderGoldSection() {
  ...
  if (cardList) {
    cardList.innerHTML = DATA.gold.map(g => `
      <div class="mc-row mc-row-card mc-row-stack">
        <div class="market-card-title">${g.name}</div>
        <div class="market-inline-row market-inline-row-wide">
          <span><span class="market-muted">Mua:</span> ${fmt(g.buy)}</span>
          <span><span class="market-muted">Bán:</span> ${fmt(g.sell)}</span>
...
function renderFuelSection() {
  ...
  if (tbody) {
    tbody.innerHTML = DATA.fuel.map((f, i) => `
      <tr data-section="fuel" data-index="${i}" class="clickable">
        ...
        <td class="market-date-cell">${f.date}</td>
      </tr>
    `).join('');
  }
  ...
  if (cardList) {
    cardList.innerHTML = DATA.fuel.map(f => `
      <div class="mc-row mc-row-card">
        <span class="market-card-name">${f.name}</span>
        <div class="market-value-stack">
          <span class="market-mono-strong">${fmt(f.price)} ${displayFuelUnit(f)}</span>
          <span class="${dirClass(f.change)} market-small-change">${fmtChange(f.change, ' đ')}</span>
        </div>
      </div>
    `).join('');
  }
...
function renderStockSection() {
  ...
  if (tbody && data) {
    tbody.innerHTML = data.map(s => `
      <tr>
        <td class="market-card-title">${s.code}</td>
        <td>${fmt(s.price)}</td>
...
function renderExchangeSection() {
  ...
  if (cardList) {
    cardList.innerHTML = DATA.forex.map(f => `
      <div class="mc-row mc-row-card">
          <span class="market-card-title">${f.currency}</span>
          <div class="market-inline-row">
            <span><span class="market-muted">MUA:</span> ${fmt(f.transfer)}</span>
            <span class="up"><span class="market-muted">BÁN:</span> ${fmt(f.sell)}</span>
          </div>
        </div>
    `).join('');
  }
  ...
  if (grid) {
    grid.innerHTML = DATA.rates.map(r => `
      <div class="rate-item">
        ...
        <span class="rate-val">${r.rate}<small class="rate-unit">%</small></span>
...
function renderForexTable() {
  ...
  if (cardList) {
    cardList.innerHTML = DATA.forex.map(f => `
      <div class="mc-row mc-row-card">
        <span class="market-card-title">${f.flag} ${f.currency}</span>
        <div class="market-inline-row">
          <span><span class="market-muted">MUA:</span> ${fmt(f.transfer)}</span>
          <span class="up"><span class="market-muted">BÁN:</span> ${fmt(f.sell)}</span>
        </div>
...
/** Fetch live fuel prices from Vietnam Fuel API */
...
function initApp() {
  ...
  console.log('Vietnam Fuel API - Khởi động thành công');
}
